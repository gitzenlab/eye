...
class aht20:
    ...
    AHT20_I2CADDR_DEFAULT = const(0x38)  # Default I2C address of aht20
    AHT20_CMD_INITIALIZE = 0xBE  # Initialization command
    AHT20_CMD_TRIGGER = const(0xAC)  # Trigger reading command
    AHT20_CMD_SOFTRESET = const(0xBA)  # Soft reset command
    AHT20_STATUS_BUSY = const(0x80)  # Status bit for busy
    AHT20_STATUS_CALIBRATED = const(0x08)  # Status bit for calibrated

    def __init__(self, i2c, address=AHT20_I2CADDR_DEFAULT):
        time.sleep_ms(20)  # 20ms wake up delay
        self._i2c = i2c
    ...
    def _trigger_measurement(self):
        self._buf[0] = self.AHT20_CMD_TRIGGER
        self._buf[1] = 0x33
        self._buf[2] = 0x00
        self._i2c.writeto(self._address, self._buf[0:3])

    def _wait_for_idle(self):
        while self.status & self.AHT20_STATUS_BUSY:
            time.sleep_ms(5)
...
BMP280_I2CADDR_DEFAULT = const(0x77)  # Default I2C address of bmp280
# Power Modes
...
class bmp280:
    ...
    def __init__(self, i2c_bus, addr=BMP280_I2CADDR_DEFAULT, use_case=BMP280_CASE_HANDHELD_DYN):
